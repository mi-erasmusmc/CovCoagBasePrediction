...
The setup script below is used to install the **CovCoagBasePrediction** package. You will need to modify this setup script as follows:
                                                                                              
- Set the `projectRootFolder` variable to the directory specific to your environment. In this example we are using `C:/CovCoagBasePrediction`. This root folder will serve a few purposes:
...
projectFolder <- "C:/CovCoagBasePrediction"
...
download.file("https://raw.githubusercontent.com/ohdsi-studies/CovCoagBasePrediction/master/renv.lock", "renv.lock")
...
# finally install the CovCoagBasePrediction package
install.packages('devtools')
devtools::install_github('ohdsi-studies/CovCoagBasePrediction')
...
You can safely continue by pressing 'y' after this prompt since the renv.lock file is downloaded from the **CovCoagBasePrediction** GitHub code repository. Once the installation is complete, you may need to restart R (if you are working outside of RStudio) and you should see this message:

````
Project 'C:/CovCoagBasePrediction' loaded. [renv 0.x.y]
````
